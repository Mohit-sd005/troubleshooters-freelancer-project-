...
export const AcceptAPI = {
  apply: (adId, developerId) => api.post(`/accept/apply?adId=${adId}&developerId=${developerId}`),
  list: (adId) => api.get(`/accept/list/${adId}`),
  select: (adId, developerId) => api.post(`/accept/select?adId=${adId}&developerId=${developerId}`),
}
